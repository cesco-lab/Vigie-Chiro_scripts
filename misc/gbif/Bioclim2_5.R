    ...
    for (k in 1:nrow(TileCoord))
    {
      if(is.na(clim_pts[k,2]))
      {
        ...
        while(Good==0)
        {
          ...
          ToModify[1,1]=coordinates(pts)[k,1]+jitter
          ToModify[2,1]=coordinates(pts)[k,1]-jitter
          ToModify[3,2]=coordinates(pts)[k,2]+jitter
          ToModify[4,2]=coordinates(pts)[k,2]-jitter
