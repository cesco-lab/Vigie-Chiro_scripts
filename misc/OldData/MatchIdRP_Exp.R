...
for (i in 1:length(unique(IdExp$ID_CIRCUIT)))
{
  ...
  if(length(NumNew)==0){NumNew=unique(IdExp$ID_CIRCUIT)[i]}
  ProtPedestre=as.numeric(NumNew)>1000
  NumNew=ifelse(as.numeric(NumNew)>1000,paste0("Vigiechiro - PÃ©destre-"
                                               ,NumNew)
    ...
    if(nrow(Dataj)>0)
    {
      ...
      for (k in 1:length(unique(Idi$PASSAGE)))
      {
        ...
        if(nrow(Datak)>0)
        {
          ...
          if(length(Partk)==1)
          {
            Canal=Particip$canal_expansion_temps[Particip$participation==Partk]
            if(Canal=="GAUCHE"){
            ...
            if(nrow(Datak)>0)
            {
              ...
              if("N° du tronçon"  %in% names(Idk))
              {
                Tronk=Idk$`N° du tronçon`
              }else{
              ...
              for (l in 1:length(unique(Tronk)))
              {
                ...
                if(nrow(Datal)>0)
                {
                  ...
                  for (m in 1:nrow(Idl))
                  {
                    ...
                    DataInt=subset(Datal,(Datal$TEMPS>(Idl$TEMPS[m]-5))&
                                     (Datal$TEMPS<(Idl$TEMPS[m]+3+ProtPedestre*10)))
...
fwrite(DataChecked,"DataCheckedExpansion2.csv",sep=";")
