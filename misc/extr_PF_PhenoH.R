...
if(!exists("FRaw"))
{
  args=""
  args[3]=0
  args[4]="D:/VigieChiro/Raw/DataLP_PF_export_58c.csv"
...
if(nrow(DataLP)>0)
{
  ...
  if(Filter=="time_int")
  {
    DataLPS=subset(DataLPS,DataLPS$PropH<=TimeFilterH)
    DataLPS=subset(DataLPS,DataLPS$PropH>TimeFilterL)
  }
  ...
  if(nrow(DataLPS)>0)
  {
    ...
    DataPF_MinSr=aggregate(DataFiable$DecsrP
                           ...
                           ,FUN=min)
    Sys.time()
    DataPF_SpNuit=cbind(DataPF_ActNuit,DataPF_ScoreNuit$x,DataPF_MinSt$x
                        ,DataPF_MinSr$x)
    colnames(DataPF_SpNuit)=c("participation","Nuit","num_micro","groupe"
                              ,"espece"
                              ,"nb_contacts",
                              "score_max","min_decalage_coucher","min_decalage_lever")
