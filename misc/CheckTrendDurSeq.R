...
for (i in 1:length(ListSp))
{
  ...
  Detaili$year=as.numeric(substr(Detaili$date_debut,7,10))
...
DetailBat$year=as.numeric(substr(DetailBat$date_debut,7,10))
DetailBat$yearScaled=scale(DetailBat$year)
mbat=glmmTMB(DurSeq~yearScaled+(1|LocaPartData)+(1|detecteur_enregistreur_type)+(1|espece)
      ,data=DetailBat)
