...
TagModel="GLMnonselect_MultiSp_DecOT22_AT81_Jour_simplifie1_CS_TR"
...
SpA1=aggregate(SpNuit$nb_contacts,by=list(SpNuit$espece),FUN=mean)
SpPos=subset(SpNuit,SpNuit$nb_contacts>0)
SpOcc=aggregate(SpPos$nb_contacts,by=list(SpPos$espece),FUN=length)
SpAbIfP=aggregate(SpPos$nb_contacts,by=list(SpPos$espece),FUN=mean)
...
#traits correlation
cor(SpNuit_SLPAGN$ThAve,SpNuit_SLPAGN$ThRange)
