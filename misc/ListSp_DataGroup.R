...
ListAll=T
FListSp="ListSp_CISTACEAE_FR_ES_IT_CH.csv"
Rank="FAMILY"
Pattern="DE_AT_BE_NL_LU"

if(ListAll)
{
  LListSp=list.files("./VigieChiro/gbifData/ListSp"
                     ,full.names=T,pattern=Pattern)
  
}else{
  LListSp=FListSp
}

for (g in 34:length(LListSp))
{
    ...
    for (h in 1:length(CountryFilter))
    {
      ...
      if(nrow(KidsCountry)>0)
      {
        ...
        if(length(DGL)>0)
        {
          ...
          if(nrow(DataGroup2)>0)
          {
            if("day" %in% names(DataGroup2))
            {
          ...
          fwrite(DataGroup2,paste0("F:/DataGroup/DataGroup2_",InfoLS[[2]],"_",CountryFilter[h],".csv"))
